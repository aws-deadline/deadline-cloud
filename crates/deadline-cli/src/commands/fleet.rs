...
async fn run_async(action: FleetAction) -> Result<(), CliError> {
    match action {
        FleetAction::List { profile, farm_id } => {
            ...
            let resp = api::list_fleets(&farm, config.as_ref()).await
                .map_err(|e| CliError::Operation(format!("Failed to get Fleets from Deadline:\n{e}")))?;
            let empty = vec![];
            let fleets = resp["fleets"].as_array().unwrap_or(&empty);
            let structured: Vec<serde_json::Value> = fleets.iter()
        ...
        FleetAction::Get { profile, farm_id, fleet_id } => {
            ...
            let resp = api::get_fleet(&farm, &fleet, config.as_ref()).await
                .map_err(|e| CliError::Operation(format!("Failed to get Fleet from Deadline:\n{e}")))?;
