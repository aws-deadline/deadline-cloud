use clap::Subcommand;
use deadline_lib::api::{client, responses::WorkerResponse, session};
                ...
                .await
            {
                ...
                Err(e) => {
                    ...
                    return Err(CliError::Operation(format!(
                        "Failed to get Workers from Deadline:\n{err_str}{suggestion}"
                    )));
                }
            };
        ...
        WorkerAction::Get {
                ...
                .await
            {
                ...
                Err(e) => {
                    ...
                    .await;
                    return Err(CliError::Operation(format!(
                        "Failed to get Worker from Deadline:\n{err_str}{suggestion}"
                    )));
                }
            };
            let worker_resp = WorkerResponse::from(resp);
            let val = serde_json::to_value(&worker_resp)
                .map_err(|e| CliError::Operation(e.to_string()))?;
