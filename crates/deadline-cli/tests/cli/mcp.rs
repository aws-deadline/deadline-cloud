...
use deadline_test_server::deadline_api::{
    cloudwatch, errors, farms, fleets, jobs, queues, sessions,
};
use rmcp::ServiceExt;
...
async fn mcp_server_lists_all_expected_tools() {
    ...
    let mut expected = vec![
        "deadline_list_farms",
        "deadline_list_queues",
        "deadline_list_jobs",
        "deadline_list_fleets",
        "deadline_list_storage_profiles_for_queue",
        "deadline_check_authentication_status",
        "deadline_get_session_logs",
        "deadline_get_job",
        "deadline_get_session",
        "deadline_list_sessions",
        "deadline_list_steps",
        "deadline_list_tasks",
        "deadline_search_jobs",
        "deadline_submit_job",
        "deadline_download_job_output",
...
async fn mcp_submit_job_dir_not_exist_returns_error() {
    ...
    args.insert(
        "job_bundle_dir".into(),
        Value::String("/nonexistent/path".into()),
    );
...
async fn mcp_submit_job_path_is_file_returns_error() {
    ...
    args.insert(
        "job_bundle_dir".into(),
        Value::String(harness.config_path.clone()),
    );
    ...
    let json = result_json(&result);
    assert!(
        json["error"].as_str().unwrap().contains("not a directory"),
        "expected 'not a directory': {json}"
    );
    client.cancel().await.unwrap();
}
...
async fn mcp_submit_job_params_not_array_returns_error() {
    ...
    let json = result_json(&result);
    assert!(
        json["error"].as_str().unwrap().contains("JSON array"),
...
async fn mcp_submit_job_no_farm_id_returns_error() {
    ...
    let json = result_json(&result);
    assert!(
        json["error"].as_str().unwrap().contains("farm_id"),
        "expected 'farm_id': {json}"
    );
    client.cancel().await.unwrap();
}
...
async fn mcp_submit_job_no_queue_id_returns_error() {
    ...
    let json = result_json(&result);
    assert!(
        json["error"].as_str().unwrap().contains("queue_id"),
        "expected 'queue_id': {json}"
    );
    client.cancel().await.unwrap();
}
...
async fn mcp_download_task_without_step_returns_error() {
    ...
    let json = result_json(&result);
    assert!(
        json["error"].as_str().unwrap().contains("step_id"),
        "expected 'step_id required': {json}"
    );
    client.cancel().await.unwrap();
}
...
async fn mcp_download_no_job_id_returns_error() {
    ...
    let json = result_json(&result);
    assert!(
        json["error"].as_str().unwrap().contains("job_id"),
        "expected 'job_id required': {json}"
    );
    client.cancel().await.unwrap();
}
...
async fn mcp_download_invalid_conflict_resolution_returns_error() {
    ...
    assert!(
        err.contains("SKIP") || err.contains("OVERWRITE") || err.contains("CREATE_COPY"),
...
async fn mcp_session_and_worker_logs_returns_both() {
    ...
    fleets::mock_assume_fleet_role_for_read(
        &harness.server,
        "farm-aaa",
        "fleet-fff",
        serde_json::json!({
    ...
    let result = client
        .call_tool(
            CallToolRequestParams::new("deadline_get_session_and_worker_logs").with_arguments(args),
        )
        .await
        .expect("call_tool should succeed at protocol level");
...
async fn mcp_session_and_worker_logs_permission_error() {
    ...
    assert_eq!(json["session_id"], "session-ddd");
    assert!(
        json["session_logs"]["error"].is_string(),
        "expected session_logs.error: {json}"
    );
    assert!(
        json["worker_logs"]["error"].is_string(),
        "expected worker_logs.error: {json}"
    );
    assert_eq!(json["worker_logs"]["count"], 0);
    client.cancel().await.unwrap();
}
...
async fn mcp_session_and_worker_logs_custom_limit() {
    ...
    let json = result_json(&result);
    assert!(
        json["session_logs"].is_object(),
        "expected session_logs: {json}"
    );
    client.cancel().await.unwrap();
}
...
async fn mcp_submit_job_params_invalid_json_returns_error() {
    ...
    let json = result_json(&result);
    assert!(
        json["error"].as_str().unwrap().contains("JSON")
            || json["error"].as_str().unwrap().contains("parse"),
...
async fn mcp_session_and_worker_logs_includes_session_details() {
    ...
    assert!(
        json["host_properties"].is_object(),
        "expected host_properties: {json}"
    );
    client.cancel().await.unwrap();
}
