...
pub async fn mock_list_jobs_paginated(
    ...
    Mock::given(method("GET"))
        ...
        .respond_with(ResponseTemplate::new(200).set_body_json(json!({ "jobs": page2 })))
        .mount(server)
        .await;
}
...
pub async fn mock_get_step(
    ...
    Mock::given(method("GET"))
        .and(path(format!(
            "/2023-10-12/farms/{farm_id}/queues/{queue_id}/jobs/{job_id}/steps/{step_id}"
        )))
