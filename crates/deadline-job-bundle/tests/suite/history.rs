...
fn history_first_submission_creates_01_dir() {
    ...
    let result =
        create_job_history_bundle_dir("TestSubmitter", "TestJob", base.path().to_str().unwrap())
            .unwrap();
    let dirname = std::path::Path::new(&result).file_name().unwrap().to_str().unwrap();
...
fn history_second_submission_creates_02_dir() {
    ...
    let second =
        create_job_history_bundle_dir("Sub", "Job", base.path().to_str().unwrap()).unwrap();
    let dirname = std::path::Path::new(&second).file_name().unwrap().to_str().unwrap();
...
fn history_submitter_special_chars_stripped() {
    let base = TempDir::new().unwrap();
    let result = create_job_history_bundle_dir(
        "Test@#$Submitter",
        "Job",
...
fn history_month_dir_created_and_reused() {
    ...
    assert!(month_name.len() == 7 && month_name.contains('-'), "Expected YYYY-MM, got: {month_name}");

    // Second call reuses the same month dir
    let second =
        create_job_history_bundle_dir("Sub", "Job2", base.path().to_str().unwrap()).unwrap();
    assert_eq!(
        std::path::Path::new(&second).parent().unwrap(),
        parent
    );
}
...
fn history_numbering_gaps_uses_max_plus_one() {
    ...
    let next =
        create_job_history_bundle_dir("Sub", "Job", base.path().to_str().unwrap()).unwrap();
    let dirname = std::path::Path::new(&next).file_name().unwrap().to_str().unwrap();
...
fn history_uses_provided_base_dir() {
    ...
    let result =
        create_job_history_bundle_dir("Sub", "Job", custom.to_str().unwrap()).unwrap();
