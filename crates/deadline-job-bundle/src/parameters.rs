...
const VALID_UI_CONTROLS: &[&str] = &[
    "CHECK_BOX", "CHOOSE_DIRECTORY", "CHOOSE_INPUT_FILE", "CHOOSE_OUTPUT_FILE",
    "DROPDOWN_LIST", "LINE_EDIT", "MULTILINE_EDIT", "SPIN_BOX", "HIDDEN",
];
...
pub fn validate_job_parameter(
    ...
    let name = match obj.get("name") {
        ...
        Some(v) => v.as_str().ok_or_else(|| op_err(format!(
...
pub fn validate_user_interface_spec(
    ...
    let obj = input.as_object().ok_or_else(|| {
        op_err(format!("Expected a dict but got {}", json_type_name(input)))
    })?;

    if let Some(v) = obj.get("control") {
        let c = v.as_str().unwrap_or("");
        if !VALID_UI_CONTROLS.contains(&c) {
            let quoted: Vec<_> = VALID_UI_CONTROLS.iter().map(|c| format!("\"{c}\"")).collect();
            return Err(op_err(format!(
    ...
    if let Some(v) = obj.get("fileFilterDefault") {
        validate_user_interface_file_filter(
            v, parameter_name, "\"userInterface\" -> \"fileFilterDefault\"",
        )?;
    }
...
pub fn validate_user_interface_file_filter(
    ...
    let obj = input.as_object().ok_or_else(|| op_err(format!(
    ...
    match obj.get("label") {
        ...
        Some(v) if !v.is_string() => return Err(op_err(format!(
...
pub fn read_job_bundle_parameters(
    ...
    let template = read_yaml_or_json_object(bundle_dir, "template", true)?
    ...
    for (name, param) in &mut params {
        ...
        if is_path && !has_value && !has_allowed
            && let Some(default) = param.get("default").and_then(|v| v.as_str()).map(String::from)
                && !default.is_empty() {
                    ...
                    let bundle_real = std::fs::canonicalize(bundle_dir).unwrap_or_else(|_| Path::new(bundle_dir).to_path_buf());
                    ...
                    if !default_real.starts_with(&bundle_real) {
                        return Err(op_err(format!(
                            ...
                            default_real.display(), bundle_real.display()
                        )));
                    }
    ...
    for p in &parameters {
        let is_hidden = p.get("userInterface")
    ...
    if !invalid.is_empty() {
        let msg = if invalid.len() == 1 {
            format!("Job bundle validation failed:\nHidden parameter \"{}\" is missing a value.", invalid[0])
        } else {
            let list = invalid.iter().map(|n| format!("\"{n}\"")).collect::<Vec<_>>().join(", ");
...
pub fn apply_job_parameters(
    ...
    for param in parameters.iter_mut() {
        ...
        if ptype == "PATH" {
            let data_flow = param.get("dataFlow").and_then(|v| v.as_str()).unwrap_or("NONE");
            ...
            if data_flow == "NONE" {
                ...
                if data_flow.contains("IN") {
                    if obj_type == Some("FILE") {
                        ...
                        asset_references.input_directories.insert(param_value.clone());
                    }
                }
                if data_flow.contains("OUT") {
                    if obj_type == Some("FILE") {
                        if let Some(parent) = Path::new(&param_value).parent() {
                            asset_references.output_directories.insert(parent.to_string_lossy().into_owned());
                        }
                    } else {
                        asset_references.output_directories.insert(param_value.clone());
                    }
                }
            }
        }
...
pub fn get_ui_control_for_parameter_definition(
    ...
    let explicit = param.get("userInterface").and_then(|ui| ui.get("control")).and_then(|c| c.as_str());
    let has_allowed = param.get("allowedValues").is_some();
    ...
    let control = if let Some(c) = explicit {
        ...
        match ptype {
            ...
            "PATH" => {
                let obj_type = param.get("objectType").and_then(|v| v.as_str()).unwrap_or("DIRECTORY");
                return Ok(if obj_type == "FILE" {
                    let df = param.get("dataFlow").and_then(|v| v.as_str()).unwrap_or("NONE");
                    if df == "OUT" { "CHOOSE_OUTPUT_FILE" } else { "CHOOSE_INPUT_FILE" }
                } else {
                    "CHOOSE_DIRECTORY"
                }.into());
            }
    ...
    let supported = match ptype {
        ...
        "PATH" => &["CHOOSE_INPUT_FILE", "CHOOSE_OUTPUT_FILE", "CHOOSE_DIRECTORY", "DROPDOWN_LIST", "HIDDEN"],
...
pub fn parameter_definition_difference(
    ...
    for field in &["name", "type", "minValue", "maxValue", "minLength", "maxLength", "dataFlow", "objectType"] {
