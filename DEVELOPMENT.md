...
| Submit a render job using `deadline bundle submit` | `deadline bundle submit <path_to_job_bundle_dir>`. Verify job bundle can be submitted successfully to the farm (and any job attachments uploaded successfully to s3). Verify correct job information appears in DCM and job can be completed successfully. | Automated: `test/integ/cli/test_cli_bundle_submit.py` |
...
| `deadline job download-output` | Verify job output can be successfully downloaded using command. | Automated: `test/integ/cli/test_cli_bundle_submit.py::test_job_download_output` |
...
| `deadline job cancel` | Verify existing/running jobs can be successfully canceled. Verify job shows as Canceled in DCM. | Automated: `test/integ/cli/test_cli_job.py::test_job_cancel` |
| `deadline job get` | Verify correct information is displayed. | Automated: `test/integ/cli/test_cli_job.py::test_job_get` |
| `deadline job list` | Verify correct information is displayed (can check DCM for info). | Automated: `test/integ/cli/test_cli_job.py::test_job_list`, `test_job_list_page_size` |
| `deadline job trace-schedule` | | Automated: `test/integ/cli/test_cli_job.py::test_job_trace_schedule` |
