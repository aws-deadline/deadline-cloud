...
"""Tests for _normalize_include_paths."""

import sys

from deadline.client.cli._groups._job_download_helpers import _normalize_include_paths


class TestValidateAndNormalizePathFilters:
    def test_converts_backslashes(self):
        result = _normalize_include_paths(["renders\\frame_001.exr"])
        assert result == ["renders/frame_001.exr"]

    def test_strips_leading_dot_slash(self):
        result = _normalize_include_paths(["./renders/frame.exr"])
    ...
    def test_empty_filter_removed(self):
        result = _normalize_include_paths(["", "a.txt"])
        assert result == ["a.txt"]

    def test_passthrough_normal_paths(self):
        result = _normalize_include_paths(
