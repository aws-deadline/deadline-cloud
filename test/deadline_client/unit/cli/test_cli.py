...
import deadline.client.cli.deadline_cli_main
from deadline.client import api
from deadline.client.cli import deadline_cli
from deadline.client.cli._common import cli_object_repr
...
def test_cli_debug_logging_on(fresh_deadline_config):
    ...
    # environment.
    cli_py = deadline.client.cli.deadline_cli_main.__file__
    output = subprocess.check_output(
        args=[sys.executable, cli_py, "--log-level", "DEBUG", "config"],
...
def test_cli_group_without_command(fresh_deadline_config, cli_group):
    ...
    """
    runner = CliRunner()
    result = runner.invoke(deadline_cli.cli, [cli_group])
...
def test_cli_object_repr(obj, expected):
    ...
    """
    assert cli_object_repr(obj) == expected
