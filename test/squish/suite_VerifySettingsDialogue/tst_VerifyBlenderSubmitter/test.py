...
def main():
    ...
    aws_submitter_helpers.verify_deadline_cloud_settings(
        config.farm_name, config.farm_desc, config.queue_name
    )
    # verify shared job settings tab contains correct Queue Environment
    aws_submitter_helpers.verify_queue_environment(
        config.blender_conda_package, config.conda_channel
    )
    ...
    aws_submitter_helpers.verify_render_parameters(
