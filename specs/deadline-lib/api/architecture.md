...
│   └── config
├── bundle    ──► api
└── attachments (independent S3/STS clients)

deadline-cli ──► deadline-lib::api
deadline-python-bindings ──► deadline-lib::api
```

All Deadline Cloud API calls are constructed and sent through this
module — it owns the session cache, credential resolution, telemetry
...
fluent builders at callsite. The helpers in `deadline-lib::api::client`
observe the call (pagination, telemetry via interceptor, error
mapping) — they do not re-declare SDK inputs.

Exception: `deadline-lib::attachments` owns its own S3 and STS clients
