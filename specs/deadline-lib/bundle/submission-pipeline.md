...
- `job_bundle_dir` — `PathBuf` to the bundle directory
...
- `known_asset_paths` — `Vec<PathBuf>` paths that should not generate warnings
- `auto_accept` — skip confirmation prompts
- `force_s3_check` — override S3 verification behavior
- `debug_snapshot_dir` — optional `PathBuf` for debug manifest snapshots
- `config` — `&IniConfig` (required, caller provides)
- `handler` — `&dyn SubmissionHandler` for user interaction
- `hashing_progress_callback` — `Option<ProgressFn>` `(processed_bytes, total_bytes) -> bool`
- `upload_progress_callback` — `Option<ProgressFn>` `(processed_bytes, total_bytes) -> bool`
- `telemetry` — optional `&TelemetryClient`

## SubmissionHandler Trait

User interaction boundary — the library returns decision points, the
caller (CLI/GUI/MCP) decides how to present them:
...
pub trait SubmissionHandler: Send + Sync {
    ...
    fn on_upload_summary(&self, stats: &SummaryStatistics);
}
```

The CLI implements this with `println!` + `indicatif` progress bars.
The GUI implements it with Qt signals. The MCP server ignores messages.
...
create_job_from_job_bundle(params)
  ...
  ├── 10. Hash assets and create manifests (via deadline-lib::attachments)
  │       Progress reported via hashing_progress_callback
  ├── 11. Upload assets to S3 CAS (via deadline-lib::attachments)
  │       Progress reported via upload_progress_callback
  ├── 12. Build CreateJob request (template + parameters + attachment metadata)
  ├── 13. Call CreateJob API (via deadline-lib::api)
