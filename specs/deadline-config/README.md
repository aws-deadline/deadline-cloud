...
Consumers: `deadline-cli`, `deadline-api`, `deadline-python-bindings`,
`deadline-job-attachments`.

## Hierarchical Setting Resolution

Settings are scoped by AWS profile → farm → queue. A setting like
`defaults.queue_id` resolves to the INI section
`"profile-MyProfile farm-abc defaults"`, key `"queue_id"`. The dependency
chain walks from the most specific section back to the default, so a
queue-level override takes precedence over a farm-level default.

The crate exposes a two-tier API: config-explicit functions (take
`&IniConfig`, no disk I/O) and convenience wrappers (read/write disk).
The CLI reads config once at startup and threads `&IniConfig` through
all calls. The convenience wrappers exist for simple scripts and the
GUI FFI layer.
