...
40. `[DESIGN]` boto3's `ClientError` formats error messages as `"An error occurred (Code) when calling the Operation operation: message"`, embedding the operation name in the string. The AWS SDK for Rust's `SdkError` `Display` impl just says `"service error"` — it never includes the error code or message. The Rust CLI uses `format_sdk_error` which extracts the code and message via `ProvideErrorMetadata`, producing `"Code: message"`. The operation name is not available from the SDK error type. This means all API error messages in the Rust CLI omit the `"when calling the X operation"` phrase. This is an inherent SDK difference, not a bug.
